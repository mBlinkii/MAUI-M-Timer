...
Data.entries = {
    {
        ...
        updates = {
            ...
            "Enemy Forces: the checkpoint countdown now behaves exactly like the timer bar's countdown - the labels anchor at the checkpoint boundaries (markers on the single bar, gap centers on the split bar) with the same position modes, and by default only the next checkpoint counts down; a new option shows all at once. Works with and without the split bar. 0% and 100% targets are ignored - the bar's own ends carry no divider, marker, or countdown.",
